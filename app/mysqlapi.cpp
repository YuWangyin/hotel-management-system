...
@param tableView 显示数据的表格对象。

@param labInfo 显示翻页信息的标签对象。

@param btnFirst 第一页按钮对象。

@param btnPre 上一页按钮对象。

@param btnNext 下一页按钮对象。

@param btnLast 末一页按钮对象。
*/
...
int MysqlApi::SearchMaxValue(QString columnName, QString tableName)
{
    ...
    if(query.next())
    {
        ...
        maxvalue = query.value(0).toInt();//将查询结果转换为整型
    }
    return maxvalue;//返回查询结果
}
/*
 *函数功能：分页绑定指定表格中的数据到tableview中
*/
void MysqlApi::BindData(QString tableName, QString orderColumn, QString orderType, int rowsCount, QString columnNames[], int columnWidths[])
{
    ...
    TableView->setStyleSheet( "QTableView::item:hover{background-color:rgb(92,188,227,200)}"
                              "QTableView::item:selected{background-color:#1B89A1}");// 设置表格样式，鼠标悬浮和选中时的背景色
}

/*
 *函数功能：绑定任意表中的数据到tableview中
 参数：
tableName：QString类型，表示需要绑定的表名
tableView：QTableView类型，表示需要绑定到的视图控件
columnNames：QString类型数组，表示需要设置的列标题名
columnWidths：int类型数组，表示需要设置的列宽度
*/
...
函数功能：绑定指定的SQL语句查询结果到tableview中
参数：
sql：QString类型，表示需要查询的SQL语句
*/
...
函数功能：绑定指定表中的数据到tableview中，实现分页功能

参数：
tableName：QString类型，表示需要绑定的表名
orderColumn：QString类型，表示按照哪个列进行排序
orderType：QString类型，表示排序类型，可以是ASC或者DESC
where：QString类型，表示SQL语句中的WHERE部分，可以为空字符串
rowsCount：int类型，表示每页显示的行数
columnNames：QString类型数组，表示需要设置的列标题名
columnWidths：int类型数组，表示需要设置的列宽度
*/
...
/*
实现数据库查询结果的分页显示功能
通过限制查询结果的行数和起始位置，实现了分页显示查询结果的功能。
*/


void MysqlApi::First()//首页
{
    if (PageCount>1)// 当页数大于1时才进行操作
    {
        StartIndex=0;
        PageCurrent=1;
        Sql=QString("%1 limit %2,%3;").arg(TempSql).arg(StartIndex).arg(RowsCount);//组织分页SQL语句
