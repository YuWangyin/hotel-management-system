...
void Widget::InitStyle()
{
    ...
    promanage = new ProManage();                        //联系小0管理界面
    constumerInfo = new CostomerRegisterInfoDialog();   //注册组员信息界面
    romminfo = new RommInfo();                         //坑位信息界面
    modifyRoom = new ModiftRoomPrice();                 //修改坑位价格界面
    checkcustom = new CheckCutomDialog();               //组员登记入住界面
    logblog = new LogBlogDialog();                      //操作日志界面
    modifypwd = new ModifyPwdDialog();                  //修改密码界面
    roompicture = new RoomPicDialog();                  //展示坑位照片界面
    backdatabase = new BackupDatanaseDialog();          //数据库备份界面
    checkoutRoom = new CheckOutDialog();                //组员蹲坑界面


    //outinfo在widget.h中定义了，上面的变量也是
    outinfo = new Out();//蹲坑记录界面
    ...
    ui->label_CompanyName->setText(tr("坑位打卡管理系统"));
    ui->label_CurrentUser->setText(QString(tr("当前组员:%1【%2】")).arg(Myapp::CurrentUserName).arg("监督官"));
...
void Widget::on_toolBox_destroyed()
{
